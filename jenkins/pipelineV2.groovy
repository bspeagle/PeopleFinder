node {
    stage("Clone repo and add .env file") {
        ...
        dir ('PeopleFinder') {
            configFileProvider([configFile(fileId: 'peopleFinder_ENV', targetLocation: '.env')]) {
                configFileProvider([configFile(fileId: 'bspeagle_PEM', targetLocatiopn: 'bspeagle.pem')]) {
                    sh 'ls'
